...
function popup(intable) {
    ...
    container.actions(
        Actions.translateBy(0, -table.getPrefHeight(), 1.0, Interp.fade),
        Actions.delay(2.5),
...
function eventLog(team, tile) {
    ...
    queue.add(
        "E-" +
            ...
            team.name +
            "[white] has placed:" +
            getConstructingBlock(tile).localizedName +
            " at [" +
            tile.x +
            "," +
            tile.y +
...
const keyModifiers = [KeyCode.altLeft, KeyCode.altRight, KeyCode.shiftLeft, KeyCode.shiftRight, KeyCode.controlLeft, KeyCode.controlRight];
const alphabetKeys = [
	KeyCode.a,
	KeyCode.b,
	KeyCode.c,
...
const bindableKeys = [];
for (var arr of [keyModifiers, alphabetKeys, numericKeys, fKeys, otherKeys]) {
	for (var key of arr) {
		bindableKeys.push(key);
	}
}
const strToKey = {};
for (var key of bindableKeys) {
	strToKey[key.value] = key;
}

function getSchemByKeys(keys) {
	return Core.settings.getString("schem-key-" + keys);
}
function bindSchemTo(name, keys) {
	if (!name) return;
	unbindSchemByKeys(keys);
	unbindSchem(name);
	print("Binding " + name + " to " + keys);
	Core.settings.put("schem-key-" + keys, name);
	Core.settings.put(name + "-schem-key", keys);
}
function unbindSchem(name) {
	if (!name) return;
	var keys = Core.settings.getString(name + "-schem-key");
	print("Unbinding " + name + " from keys " + keys);
	Core.settings.remove("schem-key-" + keys);
	Core.settings.remove(name + "-schem-key");
}
function unbindSchemByKeys(keys) {
	unbindSchem(getSchemByKeys(keys));
}
function getSchemBinding(name) {
	if (!name) return;
	return Core.settings.getString(name + "-schem-key");
}
...
Events.on(
    ...
    cons(e => {
        ...
        addTrackHandler(
            BlockTrackHandler.new("graphite", BlockBuildTracker, Blocks.graphitePress, false, {
        ...
        addTrackHandler(
            BlockTrackHandler.new("silicon", BlockBuildTracker, Blocks.siliconSmelter, false, {
                customText: function (team, block, tile) {
        ...
        addTrackHandler(
            BlockTrackHandler.new("plast", BlockBuildTracker, Blocks.plastaniumCompressor, false, {
                customText: function (team, block, tile) {
        ...
        addTrackHandler(
            BlockTrackHandler.new("phase", BlockBuildTracker, Blocks.phaseWeaver, false, {
                customText: function (team, block, tile) {
        ...
        addTrackHandler(
            BlockTrackHandler.new("surge", BlockBuildTracker, Blocks.surgeSmelter, false, {
                customText: function (team, block, tile) {
        ...
        Vars.content.blocks().each(e2 => {
            if (e2 instanceof UnitFactory) {
                ...
                e2.buildType = () => {
                    return extend(UnitFactory.UnitFactoryBuild, e2, {
                        drawables: [Object.create(unitprogressbar)],
                        draw() {
                            var replaced = false;
            ...
            if (e2 instanceof Reconstructor) {
                addTrackHandler(
                    BlockTrackHandler.new(e2.name, BlockBuildTracker, e2, false, {
                        customText: function (team, block, tile) {
	...
	Vars.ui.chatfrag = extend(	ChatFragment,{
        ...
        var coreplus = t => {
            ...
            powbar.set(
                prov(() => {
                    return "Power: " + (powerBalance() >= 0 ? "+" : "") + Strings.fixed(powerBalance() * 60.0, 1);
                }),
                floatp(() => {
                    return getBatLevel();
                }),
        ...
        coreplus(
            Vars.ui.hudGroup.find(
                boolf(e => {
                    return e instanceof CoreItemsDisplay;
                })
            )
        );
        ...
        var custominfo = extend(BaseDialog, "", {
            showSchem(schem) {
                    ...
                    cons(r => {
                        ...
                        arr.each((item, amount) => {
                            ...
                            r.label(() => {
                                ...
                                return (
                                    (core.items.has(item, amount) ? "[lightgray]" : "[scarlet]") +
                                    Math.min(core.items.get(item), amount) +
                                    "[lightgray]/" +
                                    amount
                                );
                            })
                    ...
                    this.cont.table(
                        cons(t => {
                            if (!Mathf.zero(prod)) {
                                ...
                                t.add("+" + Strings.autoFixed(prod, 2))
                            ...
                            if (!Mathf.zero(consump)) {
                                ...
                                t.add("-" + Strings.autoFixed(consump, 2))
                                    .color(Pal.remove)
                                    .left();
                            }
                        })
                    ...
                    cons(tbl => {
                        for (let i = 0; i < 10; i++) {
                            ...
                            tbl.button(
                                ...
                                .update(b => {
                                    b.setChecked(Core.settings.getString(g + "-schem") == schem.name());
                                })
                                .width(width)
                                .height(height)
                                .name("test" + 1)
                                .tooltip("set to slot " + g);
                        }
                    })
                );
                this.cont.row();
                ...
                this.cont
                    .button(
                        "Bind: " + getSchemBinding(schem.name),
                        Styles.defaultt,
                        run(() => {
                            if (isRebinding) {
								...
								if (keysStr != null) {
									...
									pressedKeys.length = 0;
									for (var keyStr of keysStr) {
										pressedKeys.push(strToKey[keyStr]);
									}
								}
							}
                    ...
                    .update(b => {
                        var tempKeys = getPressedKeys();
                        if (tempKeys.length > 0) {
                            var modifiersOnly = true;
                            for (var key of tempKeys) {
                                if (!keyModifiers.includes(key)) {
                                    modifiersOnly = false;
                                    break;
                                }
                            }
                        ...
                        if (isRebinding) {
							...
							if (existingSchem != null && existingSchem != schem.name()) {
								...
								b.setText(value + " is bound for " + existingSchem + " (replace?)");
							} else {
								if (value != getSchemBinding(schem.name())) {
        ...
        Vars.ui.hudGroup.fill(
            cons(t => {
                ...
                t.button(
                    new TextureRegionDrawable(rangeicon),
                    togglestyle,
                    run(() => {
                        viewAirRange = !viewAirRange;
                    })
                )
                    .update(b => b.setChecked(viewAirRange))
                    .width(46)
                    .height(46)
                    .name("airrange")
                    .tooltip("view air turret range");

                t.button(
                    new TextureRegionDrawable(rangeicon2),
                    togglestyle,
                    run(() => {
                        viewGroundRange = !viewGroundRange;
                    })
                )
                    .update(b => b.setChecked(viewGroundRange))
                    .width(46)
                    .height(46)
                    .name("groundrange")
                    .tooltip("view ground turret range");

                t.button(
                    new TextureRegionDrawable(rangeammoicon),
                    togglestyle,
                    run(() => {
                        ignoreNoAmmo = !ignoreNoAmmo;
                    })
                )
                    .update(b => b.setChecked(ignoreNoAmmo))
                    .width(46)
                    .height(46)
                    .name("ammorange")
                    .tooltip("ignore turrets without ammo");
                ...
                t.button(
                    ...
                    .height(46)
                    .name("units")
                    .tooltip("count enemy units");

                t.button(
                    new TextureRegionDrawable(progressicon),
                    togglestyle,
                    run(() => {
                        viewprogress = !viewprogress;
                    })
                )
                    .update(b => b.setChecked(viewprogress))
                    .width(46)
                    .height(46)
                    .name("progress")
                    .tooltip("show progress bar on unit factories");
                ...
                t.button(
                    ...
                    .update(b => b.setChecked(stealUnit))
                    .width(46)
                    .height(46)
                    .name("stealunit")
                    .tooltip("control nearby unit as soon as it exits factory");

                t.row();
                t.button(
                    new TextureRegionDrawable(oreicon),
                    togglestyle,
                    run(() => {
                        orescan = !orescan;
                    })
                )
                    .update(b => b.setChecked(orescan))
                    .width(46)
                    .height(46)
                    .name("ores")
                    .tooltip("show covered ores");
                t.button(
                    Icon.eyeSmall,
                    togglestyle,
                    run(() => {
                ...
                t.button(
                    new TextureRegionDrawable(pipbuttonicon),
                    togglestyle,
                    run(() => {
                        showpips = !showpips;
                    })
                )
                    .update(b => b.setChecked(showpips))
                    .width(46)
                    .height(46)
                    .name("light")
                    .tooltip("show pips");
                ...
                t.button(
                    ...
                    .tooltip("sync");
                t.button(
                    new TextureRegionDrawable(votekick),
                    style,
                    run(() => {
                ...
                t.button(
                    ...
                    .update(b => b.setChecked(!!playerAI))
                    .width(46)
                    .height(46)
                    .name("ores")
                    .tooltip("become gamma Ai");
            ...
            cons(t => {
                ...
                for (let h = 0; h < 10; h++) {
                    ...
                    let imgbutton = t
                        ...
                        .update(b => b.setDisabled(!Core.settings.getString(i + "-schem")))
                        .width(width)
                        .height(width)
                        .name("ores")
                        .tooltip("use Schem " + i)
    ...
    updateMovement() {
        ...
        if (this.mining) {
            if (this.timer.get(1, 240) || this.targetItem == null) {
                this.targetItem = unit.team.data().mineItems.min(
                    boolf(i => Vars.indexer.hasOre(i) && unit.canMine(i)),
                    floatf(i => core.items.get(i))
                );
            }
...
Events.run(Trigger.update, () => {
    ...
    if (wasCleared) {
        ...
        while (!queue.isEmpty()) {
            try {
                Vars.ui.chatfrag.addMessage("[red]PvP-Alerts: " + queue.removeFirst());
            } catch (e) {
                try {
                    Vars.ui.chatfrag.addMessage(queue.removeFirst(), "[red]PvP-Alerts");
                } catch (e2) {
                    // ignore
                }
            }
	...
	var tempKeys = getPressedKeys();
	var value = tempKeys.length == 0 ? "..." : tempKeys.map(k => k.value).reduce((a, b) => a + "+" + b);
	var schemName = getSchemByKeys(value);
	if (schemName != null) {
		print("Detected pressed keys: " + value);
		if (!useSchematic(schemName)) {
			unbindSchem(schemName);
		}
	}
...
function update() {
    if (!queue.isEmpty()) {
        if (enabled) {
            if (prevsent > 120) {
                Call.sendChatMessage(prefix + " " + queue.removeFirst());
                prevsent = 0;
            }
        } else {
            try {
...
global.alerts.onChat = function (msg) {
    onChat("Xelo", msg);
};
